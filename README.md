...
Скриншоты пути `take` (главная → подбор → анкета → отправка → ЛК) и `create`
(реестр → конструктор → live-превью → AI-генератор) прикладываются к заявке
отдельным PDF/архивом — см. `screenshots/` (заполняется перед подачей) и
презентацию PDF, обязательный артефакт заявки (SPEC.md §13).
