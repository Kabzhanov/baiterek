...
def dependencies(expr: Any) -> set[str]:
    ...
    if isinstance(expr, dict):
        return set().union(*(dependencies(v) for v in expr.values()))
    if isinstance(expr, list):
        return set().union(*(dependencies(v) for v in expr))
    return set()
...
def topological_order(graph: dict[str, set[str]]) -> list[str]:
    ...
    while graph:
        ...
        if not ready:
            raise ValueError("formula dependency cycle")
        result.extend(ready)
        for key in ready:
            graph.pop(key)
...
def evaluate_formula(expr: Any, data: dict[str, Any]):
    ...
    def evaluate(item):
        ...
        if not isinstance(item, dict):
            return Decimal(str(item)) if isinstance(item,(int,float,Decimal)) and not isinstance(item,bool) else item
        op = item.get("op")
        args = [evaluate(a) for a in item.get("args", [])]
        if op == "add":
            ...
            result = sum((Decimal(str(v)) for v in args[0]), Decimal(0))
        elif op == "min":
            result = min(args)
        elif op == "max":
            result = max(args)
        elif op == "round":
            result = args[0].quantize(Decimal(1).scaleb(-int(args[1])), rounding=ROUND_HALF_UP)
        elif op == "if":
            result = args[1] if bool(args[0]) else args[2]
        elif op == "annuity":
            principal, annual, months = args
