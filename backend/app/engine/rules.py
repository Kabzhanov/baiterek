...
def evaluate_condition(expr: dict, data: dict[str, Any]) -> RuleResult:
    ...
    def value(item): return data.get(item[1:]) if isinstance(item, str) and item.startswith("$") else item
    if op in {"and", "or"}:
    ...
    try: result = bool(operations[op]())
    except TypeError as exc: raise ValueError(f"invalid operands for {op}") from exc
    return RuleResult(result, (f"{op}({left!r},{right!r})={result}",))

def effects(rules, data):
    result, trace = {}, []
    for rule in rules:
        evaluated = evaluate_condition(rule.when, data); trace.extend(evaluated.trace)
        if evaluated.value: result[rule.target] = rule.effect
    return result, trace
