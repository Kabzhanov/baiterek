...
def definition(**updates):
    ...
    source.update(updates); return ServiceDefinition.model_validate(source)

def test_schema_round_trip_and_json_schema():
    item=definition(); assert ServiceDefinition.model_validate_json(item.model_dump_json()) == item
...
def test_semantic_validation_rejects_bad_reference_and_cycle():
    with pytest.raises(ValidationError,match="unknown formula"): definition(computed=[{"key":"x","expression":{"op":"add","args":["$missing",1]}}])
    with pytest.raises(ValidationError,match="cycle"): definition(computed=[{"key":"x","expression":{"op":"add","args":["$y",1]}},{"key":"y","expression":{"op":"add","args":["$x",1]}}])

def test_render_validation_and_lifecycle_are_generic():
    item=definition(); screen=render(item,{"age":20})
    ...
    with pytest.raises(ValueError): transition(item,"submitted","draft",{})
