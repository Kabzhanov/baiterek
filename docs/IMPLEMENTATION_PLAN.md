...
**Статус:** утверждён владельцем 07.07.2026  
**Дата:** 07.07.2026 · **Дедлайн:** 12.07.2026  
**GitHub:** https://github.com/Kabzhanov/baiterek  
**Ветка последующих push:** `main`  
**Публичный стенд (канонический URL, поддомены не используются):** `https://bizdnai.com/baiterek/`  
...
frontend/app/{take,create}          # basePath /baiterek задан в next.config.ts
...
- CI: backend tests, frontend lint/typecheck/build, secret scan,
  grep-инвариант hardcode («вагон», «животновод», wagons, agroanimal в
  `backend/app`/`frontend` вне seed-фикстур → fail; страховка дисквал-условия).
...
Checkpoint адресует экран `screen_key` (ключ первого поля экрана), не порядковым
индексом: перерасчёт видимости не должен смещать сохранённую позицию; исчезнувший
экран → ближайший существующий.
...
- POST applications идемпотентно возвращает active draft; если после публикации
  v2 у пользователя жив draft на v1 — возвращается он, второй draft не создаётся;
...
- суточный потолок расходов LLM (`LLM_DAILY_BUDGET`) на публичном стенде:
  при исчерпании — graceful-фолбэк на MockLLM с пометкой в UI;
...
Ядро (rules/formulas/validator/lifecycle) покрыто полностью, включая property-based;
процент coverage вторичен и не является самоцелью. Frontend typecheck/build и
P0 E2E на стенде зелёные.
...
безопасный доступ так: фиксированный test-author аккаунт (роль `author`, отдельный
демо-тенант), код доступа публикуется в README/материалах подачи; сид демо-данных
пересоздаётся автоматически каждую ночь (cron reset), чтобы эксперименты жюри не
портили стенд. Mock-действия подписаны. README содержит путь проверки за 5 минут,
screenshots и ограничения.
...
### 11.07 — модули, hardening и презентация

Карта/аналитика/materials; security tests; E2E; [VERIFY]; backup/restore; runbook.
**Презентация PDF (черновик всех слайдов) + screencast AI-генератора 60–90 сек** —
питч-критерии 9.8–9.10 дают 35 баллов, на 12.07 их оставлять нельзя.
Контроль: release candidate и scope freeze.

### 12.07 — выпуск

Production build/migrations; external/mobile smoke; screenshots/docs; secret scan;
финализация презентации PDF (только правки, не создание); после утверждения tag
`v0.1.0-submission`. Новые функции запрещены.
...
| Публичный стенд выжигает бюджет LLM | rate limit + `LLM_DAILY_BUDGET` + фолбэк MockLLM |
| Checkpoint смещается при изменении видимости | адресация экрана по `screen_key`, не индексом |
